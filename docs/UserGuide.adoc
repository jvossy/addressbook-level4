...
Clicking the button opens a prompt to enter details for a person. Filling in those details and clicing 'Add'
will add the new person to the list. Tags need only be separated by spaces, and the prompt informs you which
...
Restores the address book to the state before the previous _undoable_ command was executed.
Fails if there are no undoable commands executed previously.
